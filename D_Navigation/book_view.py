"""
DATEI: D_Navigation/book_view.py
PROJEKT: MyBook-Management (v1.6.19)
BESCHREIBUNG:
    - Umbenannt von BrowserView zu BookView.
    - Vollständige Integration der Enterprise Core-Atome.
    - Unterstützung für Sets (Keywords, Regions) in der Anzeige.
"""
...
class BookView:
    ...
    def fill_widgets(self, core):
        """Mappt das Core-Aggregat auf die UI."""
        if not core: return

        # 1. Pfad-Anzeige (Oben)
        self.path_entry.config(state='normal')
        self.path_entry.delete(0, tk.END)
        self.path_entry.insert(0, core.book.path)
        self.path_entry.config(state='readonly')

        # 2. SERIE
        if hasattr(core, 'serie') and core.serie:
            s = core.serie
            for l in ['de', 'en', 'fr', 'es', 'it']:
                self._set_val(f's_name_{l}', getattr(s, f'name_{l}', ""))
            # Serie Combobox befüllen
            if hasattr(core, 'all_available_series'):
                self.widgets['s_name']['values'] = core.all_available_series
            self.widgets['s_name'].set(s.name if s.name else "")

        # 3. WERK (Leading Object ist Buch -> Werk Felder sind Readonly)
        if hasattr(core, 'work') and core.work:
            w = core.work
            ...
            if hasattr(core, 'all_available_works'):
                self.widgets['w_title']['values'] = core.all_available_works
        ...
        # 4. BUCH-DETAILS
        if hasattr(core, 'book') and core.book:
            b = core.book
            ...
            if hasattr(b, 'authors') and b.authors:
                names = [f"{a[0]} {a[1]}".strip() if isinstance(a, (list, tuple)) else str(a) for a in b.authors]
    ...
    # ----------------------------------------------------------------------
    def get_data_from_widgets(self, core):
        """
        ...
        if hasattr(core, 'book') and core.book:
            ...
            try:
                ...
                b.series_index = 0.0
            if core.work:
                core.work.series_index = b.series_index
