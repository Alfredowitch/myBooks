...
from A_Captain.book_browser import BookBrowser
...
class BookAnalyser:
    ...
    def open_in_browser(self):
        """Öffnet die gefundenen Problemfälle direkt im BookBrowser."""
        ...
        with self._get_conn() as conn:
            for i in items:
                ...
                # Logik zur ID-Extraktion (gekürzt für Übersicht)
                if "ID" in cols:
                    id_list.append(vals[cols.index("ID")])
                # ... restliche ID-Logik ...

            unique_ids = list(dict.fromkeys(id_list))
            if unique_ids:
                # IMPORT CHECK: Pfad zum Browser anpassen
                if self.browser_instance and tk.Toplevel.winfo_exists(self.browser_instance.master):
                    self.browser_instance.load_ids(unique_ids)
                else:
                    top = tk.Toplevel(self.master)
                    # Wir übergeben die IDs an den neuen Browser
                    self.browser_instance = BookBrowser(top, initial_ids=unique_ids)
            else:
                messagebox.showinfo("Info", "Keine IDs zum Analysieren gefunden.")

    # --- DIE NEUE SERIEN-LOGIK (Ersetzt deinen get_or_create_work Legacy-Code) ---

    def heal_work_assignment(self, book_id, title, author_name):
        """
        Diese Methode nutzt jetzt den WorkService, um ein Buch
        einem Werk sauber zuzuordnen (Heilungsprozess).
        """
        try:
            # Wir nutzen die neue Engineering-Power
            new_work_id = Eng.Works.get_or_create_work_by_metadata(title, author_name)

            # Buch in der DB aktualisieren
            Database.execute("UPDATE books SET work_id = ? WHERE id = ?", [new_work_id, book_id])
            logging.info(f"✅ Buch {book_id} erfolgreich Werk {new_work_id} zugeordnet.")
            return True
        except Exception as e:
            logging.error(f"❌ Heilung fehlgeschlagen: {e}")
            return False

# Works - Serien Logik
    def get_or_create_work(self, title, author_name, series_name=None, series_index=0.0):
        """
        Findet ein existierendes Werk basierend auf der Identitäts-Regel:
        Gleicher Titel + Gleicher Autor + Gleicher Serien-Index.
        """
        with self.get_connection() as conn:
            # 1. Autor finden oder anlegen
            # (Hier sollte deine Logik zum Splitten von "Vorname Nachname" greifen)
            author_id = self.get_or_create_author(author_name)

            # 2. Serie finden oder anlegen (pro Autor getrennt!)
            series_id = None
            if series_name:
                series_id = self.get_or_create_series(series_name, author_id)

            # 3. Werk-Suche: Die "Heilige Dreifaltigkeit" deiner Identität
            # Wir suchen ein Werk, das denselben Titel, Index und mindestens diesen Autor hat.
            query = """
                ...
                WHERE w.title = ? 
                  ...
                  AND wta.author_id = ?
            """
            cursor = conn.execute(query, (title, series_index, author_id))
            row = cursor.fetchone()

            if row:
                logging.debug(f"✅ Werk gefunden (ID: {row[0]})")
                return row[0]
            else:
                # 4. Neu anlegen, wenn kein Match
                logging.info(f"🆕 Neues Werk wird erstellt: {title} (#{series_index})")
                cursor = conn.execute(
                    "INSERT INTO works (title, series_id, series_index) VALUES (?, ?, ?)",
                    (title, series_id, series_index)
                )
                new_work_id = cursor.lastrowid

                # Verknüpfung zum Autor herstellen
                conn.execute(
                    "INSERT INTO work_to_author (work_id, author_id) VALUES (?, ?)",
                    (new_work_id, author_id)
                )
                return new_work_id
